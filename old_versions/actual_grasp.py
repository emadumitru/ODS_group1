...
def heuristic_node_selection(G, partitioning, district_populations, nonvalid_partitions, previous_states):
    ...
    for node, current_district in partitioning.items():
        for potential_district in set(partitioning.values()):
            if potential_district == current_district or (node, potential_district) in previous_states:
                continue

            new_partitioning = partitioning.copy()
            new_partitioning[node] = potential_district
            if not check_subgraphs_connectivity(create_subgraphs_from_partitioning(G, new_partitioning)):
                continue

            # Calculate the variance in population distribution
            new_populations = populations.copy()
            node_population = G.nodes[node]['population']
            new_populations[current_district] -= node_population
            new_populations[potential_district] += node_population
            variance = np.var(list(new_populations.values()))
            
            candidates.append((variance, (node, potential_district)))

    # Randomly select a subset of candidates if too large
    if len(candidates) > 10:
        candidates = np.random.sample(candidates, 10)
    
    # Select the best candidate based on variance
    best_move = min(candidates, key=lambda x: x[0])[1] if candidates else (None, None)
...
def local_search(G, partitioning):
    """
    Perform a local search to improve the partitioning based on variance of district populations.
    """
    improvement = True
    while improvement:
        improvement = False
        district_populations = calculate_district_populations(G, partitioning)
        best_move, _, _ = heuristic_node_selection(G, partitioning, district_populations, [], [])
        if best_move[0] is not None:
            partitioning[best_move[0]] = best_move[1]
            improvement = True
    return partitioning

def heuristic_partitioning(G, num_partitions):
    """
    Creates an initial partitioning of the graph nodes into districts, aiming to balance
    the total population across partitions.
    
    Parameters:
    - G: A NetworkX graph where nodes have a 'population' attribute.
    - num_partitions: The desired number of partitions.
    
    Returns:
    - partitioning: A dictionary mapping each node to a partition.
    """
    # Initialize partitioning with all nodes unassigned
    partitioning = {node: None for node in G.nodes()}
    
    # Initialize partitions with zero population
    partition_populations = {i: 0 for i in range(num_partitions)}
    
    # List of nodes to be processed, shuffled for randomization
    nodes = list(G.nodes(data=True))
    np.random.shuffle(nodes)
    
    for node, data in nodes:
        # Assign each node to the partition with the lowest total population
        min_population_partition = min(partition_populations, key=partition_populations.get)
        partitioning[node] = min_population_partition
        partition_populations[min_population_partition] += data.get('population', 0)
    
    return partitioning

def graph_partitioning(G, num_partitions, iterations=10):
    """
    Graph partitioning using GRASP methodology, integrating local search and multiple iterations.
    
    Parameters:
    - G: A NetworkX graph.
    - num_partitions: Desired number of partitions.
    - iterations: Number of GRASP iterations to perform.
    
    Returns:
    - The best partitioning found across all iterations.
    """
    best_partitioning = None
    best_variance = np.inf

    for _ in range(iterations):
        # Randomize nodes for diverse initial solutions
        nodes = list(G.nodes())
        np.random.shuffle(nodes)
        initial_partitioning = create_initial_partition(G, num_partitions)

        # The heuristic_node_selection function inside this will include randomization for the GRASP construction phase
        partitioning = heuristic_partitioning(G, num_partitions)  # Assuming you have a heuristic function to create an initial solution
        
        # Apply local search to improve the initial solution
        partitioning = local_search(G, partitioning)

        # Evaluate the new solution
        district_populations = calculate_district_populations(G, partitioning)
        variance = np.var(list(district_populations.values()))
